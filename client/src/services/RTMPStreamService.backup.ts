...
class RTMPStreamService {
  ...
  private silenceGenerator: OscillatorNode | null = null;
  ...
  private async startMediaRecorder(): Promise<void> {
    ...
    // Criar gerador de silêncio para manter o stream de áudio estável
    // Isso evita problemas quando não há vídeo (só imagens)
    this.silenceGenerator = this.streamAudioContext.createOscillator();
    this.silenceGain = this.streamAudioContext.createGain();
    this.silenceGain.gain.value = 0.0001; // Quase inaudível mas mantém o stream ativo
    this.silenceGenerator.connect(this.silenceGain);
    this.silenceGain.connect(this.audioMixerDestination);
    this.silenceGenerator.start();
    console.log('[RTMPStreamService] ✅ Silence generator started (keeps audio stream stable)');
    ...
    this.mediaRecorder.onerror = (event) => {
      ...
      // NÃO atualizar status para error - o MediaRecorder pode continuar funcionando
      console.warn('[RTMPStreamService] MediaRecorder error occurred but continuing...');
    };

    // Start recording with optimized timeslice
    // 250ms balances latency vs CPU usage (100ms was too aggressive)
  ...
  async stopStreaming(): Promise<void> {
    ...
    if (this.silenceGenerator) {
      try {
        this.silenceGenerator.stop();
        this.silenceGenerator.disconnect();
      } catch (e) { /* ignore */ }
    ...
    this.audioMixerDestination = null;
    this.currentAudioSource = null;
